...
class Tester
{
	...
	public static function buildProject($path, &$errors)
	{
		...
		if (!empty($proFile) && (strnatcasecmp(Tester::$main_builder,"qmake") == 0 
								 || strnatcasecmp(Tester::$main_builder,"any") == 0)) {
			if (Tester::$linux_client) {
				$comand = "qmake";
			} else {
				$comand = '"' .  Tester::$path_to_QMake . '\\qmake.exe"';
			...
			Tester::readErrorOnFile("./qmakeError.txt", $header, $qmakeEr);

		}
		if(strnatcasecmp(Tester::$main_builder,"cmake") == 0 || (strnatcasecmp(Tester::$main_builder,"any") == 0 
																 && count($qmakeEr)!=0)){
			// Если pro файл собрался с ошибками
			...
			if ($cmake_path != null) {
				...
				Tester::readErrorOnFile("./cmakeError.txt", $header, $cmakeEr);
			}
		} 
		if(strnatcasecmp(Tester::$main_builder,"qmake") == 0)
		{
			$errors=array_merge($errors,$qmakeEr);
		} else if(strnatcasecmp(Tester::$main_builder,"cmake") == 0){
			$errors=array_merge($errors,$cmakeEr);
		}else if(strnatcasecmp(Tester::$main_builder,"any") == 0){
